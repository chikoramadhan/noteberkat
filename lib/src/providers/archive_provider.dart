...
class ArchiveProvider extends MainProvider {
  ...
  Future<ChatModel?> loadDetail() async {
    ...
    model = await repository.loadArchive(chatModel: chat!);
  ...
  loadTotal({required Map<String, dynamic> param, required int time}) {
    ...
    baru = -1;
    notifyListeners();
    repository.getTotalArchive(param: param).then((value) {
      if (time == _current) {
        total = value.total;
        baru = value.baru;
        notifyListeners();
  ...
  loadData({required Map<String, dynamic>? param, required int time}) {
    ...
    repository.fetchAllArchive(param: param, ignoreNull: true).then((value) {
      if (time == _current) {
        loading = false;
        updateData(value);
      }
  ...
  Future restoreChat(List<ChatModel?> chat) async {
      ...
      repository.me(user: user).then((data) async {
        ...
        if (data!.user!.blocked == null || data.user!.blocked == false) {
          prefs
            ...
            repository.fetchAllArchive(
                ignoreNull: true,
                param: {kParamSort: kSortDateDesc}).then((value) {
              updateData(value);
              loadFilterData();
            });
          });
